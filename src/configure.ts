...
import * as LanguageSelect from './examples/LanguageMultiSelect'
import { DescriptionTooltip } from './examples/DescriptionTooltip'
...
import * as xone from './examples/XoneRenderer'
...
export const componentSets: Record<ComponentsState['components'], Record<string, Component>> = {
  ...
  material: { ...nativeComponents, ...mwcComponents, languages: LanguageSelect.component('material') },
  vaadin: { ...nativeComponents, ...vaadinComponents, languages: LanguageSelect.component('lumo') },
}

editors.addMetadata([...LanguageSelect.metadata(), ])
editors.addMatchers({
  ...
  return {
    async switchNesting({ nesting }: RendererState) {
      ...
      if (nesting === 'always one') {
        const { topmostFocusNodeFormRenderer } = await import('./examples/NestedShapesIndividually/renderer')
        const nestingComponents = await import('./examples/NestedShapesIndividually/components')
        ...
        const nestingComponents = await import('./examples/InlineNestedShapes')
        components.pushComponents(nestingComponents)
      } else {
